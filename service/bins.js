...
async function meta({ binid, private = false, uid, createdAt = _.now() }) {
    ...
    await DB.put(`${binid}${META_NS}`, JSON.stringify(bin));
    return bin;
}

async function getMeta(binid) {
    try {
        let meta = await DB.get(`${binid}${META_NS}`);
...
async function getBins(binid) {
    ...
    try {
        let bins = await DB.get(`${binid}${BINS_NS}`);
...
async function bin({ binid, method = 'GET', pathname, version = '1.1', headers, query, body, ip, createdAt = _.now() }) {
    ...
    await DB.put(`${binid}${BINS_NS}`, JSON.stringify(bins));
    return bins;
}
...
async function del(binid) {
    ...
    await Promise.all([
        DB.del(`${binid}${META_NS}`),
        DB.del(`${binid}${BINS_NS}`)
    ]);
}
